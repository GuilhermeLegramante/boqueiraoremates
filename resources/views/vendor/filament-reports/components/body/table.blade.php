...
border-bottom: 1px solid rgb(210, 210, 210);">
    ...
    <x-filament-reports::table.head>
            ...
            <x-filament-reports::table.row>
                ...
                @else
                    @foreach ($columns as $column)
                        @php
                            ...
                            if (!$alignment instanceof \Filament\Support\Enums\Alignment) {
                                $alignment = filled($alignment)
                                    ? \Filament\Support\Enums\Alignment::tryFrom($alignment) ?? $alignment
                                    : null;
                            }
                        ...
                        @endphp
                        <x-filament-reports::table.head-cell class="{{ $columnClasses }}"
                            style="
                                    padding-left: 8px;
                                    padding-right: 8px;
                                    padding-top: 4px;
        ...
        @foreach ($this->heads as $item)
            <x-filament-reports::table.head-cell class=""
                style="
                                    padding-left: 8px;
                                    padding-right: 8px;
                                    padding-top: 4px;
    ...
    <x-filament-reports::table.body>
        @foreach ($data as $rowIndex => $row)
            <x-filament-reports::table.row>
                @foreach ($row->getAttributes() as $key => $cell)
                        ...
                        @if ($column->areRowsGrouped())
                            @php
                                $rowspan = $column->areRowsGrouped()
                                    ? $getNumberOfGroupedCells($rowIndex, $key, $cell)
                                    : 1;
                            @endphp
                            @if ($isFirstWithinGroup($rowIndex, $key, $cell))
                                <x-filament-reports::table.cell rowspan="{{ $rowspan }}"
                                    style="
                                        padding-left: 8px;
                                        padding-right: 8px;
                                        padding-top: 4px;
                                        ...
                                        {{ $loop->parent->last ? 'border-bottom: 1px solid #aaa;' : 'border-bottom: 1px solid #eee;' }}
                                        ">
                                    <x-filament-tables::columns.column :column="$column" :is-click-disabled="$column->isClickDisabled()"
                                        :record="$row" :record-key="$loop->iteration" />
                        ...
                        @else
                            <x-filament-reports::table.cell
                                style="
                                    ...
                                    {{ $loop->parent->last ? 'border-bottom: 1px solid #aaa;' : 'border-bottom: 1px solid #eee;' }}
                                    ">
                                <x-filament-tables::columns.column :column="$column" :is-click-disabled="$column->isClickDisabled()" :record="$row"
        ...
        <x-filament-reports::table.foot>
            <x-filament-reports::table.row>
                @foreach ($columns as $column)
                    ...
                    @endphp
                    @if ($column->hasSum())
                        <x-filament-reports::table.cell class="{{ $columnClasses }}"
                            style="
                                padding-left: 8px;
                                padding-right: 8px;
                                padding-top: 4px;
